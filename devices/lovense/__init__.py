...
from devices.lovense.shared_nodes import make_battery_node

# Combine all device classes
ALL_DEVICE_CLASSES = _VIB_DEV + _ADV_DEV

# Generate battery nodes for all devices, then combine
_battery_nodes = [make_battery_node(cls) for cls in ALL_DEVICE_CLASSES]
ALL_NODE_CLASSES = _VIB_NOD + _ADV_NOD + _battery_nodes
