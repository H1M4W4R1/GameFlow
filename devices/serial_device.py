...
class SerialSetIntensityNode(DeviceNodeBase):
    NODE_NAME       = "Serial: Set Intensity"
    NODE_GROUP      = "Serial Device"
    DEVICE_TYPE_KEY = DEVICE_TYPE_KEY
    ICON_PATH       = SerialDevice.ICON_PATH
    PINS = [
...
class SerialGetSensorNode(DeviceNodeBase):
    NODE_NAME       = "Serial: Get Sensor"
    NODE_GROUP      = "Serial Device"
    DEVICE_TYPE_KEY = DEVICE_TYPE_KEY
    ICON_PATH       = SerialDevice.ICON_PATH
    PINS = [
        PinDescriptor("exec_in",      PinDirection.INPUT,  PinType.TICK),
        PinDescriptor("sensor_value", PinDirection.OUTPUT, PinType.FLOAT),
        PinDescriptor("exec_out",     PinDirection.OUTPUT, PinType.TICK),
    ]

    def execute(self, trigger_pin: str) -> None:
        def _on_result(value: Any) -> None:
            self.set_output("sensor_value", float(value))
            self.fire_tick("exec_out")

        self.send_to_device("get_sensor", on_success=_on_result)
